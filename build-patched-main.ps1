...
async function pigmaApplyContainerClipToBackground(e,t=0,n=0){let r={applied:!1,linkedFiles:[],warnings:[]};if(!Array.isArray(e)||e.length<2)return r;let i=pigmaFindContainerClipBaseIndex(e);if(i<0)return r;let a=e[i];if(pigmaIsLayerGroup(a)){let l=await pigmaBuildClipBaseSmartObject(a,t,n);if(!l){r.warnings.push("\"".concat(Su(a&&a.name?a.name:"clip base"),"\" stayed as a group, so Pigma skipped automatic clipping masks for this container to avoid hiding clipped layers in Photoshop."));return r}e[i]=l.layer,r.linkedFiles.push(...l.linkedFiles),r.warnings.push(...l.warnings)}for(let l=i+1;l<e.length;l+=1){let u=e[l];if(u&&typeof u=="object"){u.clipping=!0,r.applied=!0}}return r}
async function vm(
'@
